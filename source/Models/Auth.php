...
class Auth extends Model
{
    ...
    {
        parent::__construct("users", ["id"], ["email", "password", "userdata"]);
    }
     ...
     * @return bool
     */
    public function login(string $email, string $password, bool $save = false, int $role = 1): bool
    {
        if (!is_email($email)) {
    ...
    public function saveUserData(string $email)
    {
        $user =  (new User())->findByEmail($email);
        ...
        $array['is_ldap_user'] = isset($array['is_ldap_user']) ? (bool) $array['is_ldap_user'] : false;
        $array['twofactor_activated'] = isset($array['twofactor_activated']) ? (bool) $array['twofactor_activated'] : false;
        $array["userdata"] = [];

        $encoded = serialize($array);
        $user->userdata = $encoded;
        $user->save();
    ...
    {
       $user = (new User())->findByEmail($email);
       $decoded = unserialize($user->userdata);
       var_dump($decoded);
    }

    public function logOutKb()
    {
        $user = Auth::user();
        $user->userdata = null;
        $user->save();
